...
struct FastIO {
    ...
    FastIO() {
        struct stat sb;
        fstat(0, &sb);
        p_in = (char*)mmap(nullptr, sb.st_size, PROT_READ, MAP_PRIVATE, 0, 0);
        p_out = out_buf;
        p_out_end = out_buf + (1 << 16);
    }
    ~FastIO() { flush(); }
    ...
    int read() {
        int x = 0;
        bool neg = false;
        while (isspace(*p_in)) p_in++;
        if (*p_in == '-') neg = true, p_in++;
        while (isdigit(*p_in)) x = x * 10 + (*p_in++ - '0');
        return neg ? -x : x;
    }
    void write(int x) {
        if (p_out + 20 > p_out_end) flush();
        if (x < 0) *p_out++ = '-', x = -x;
        char temp[20];
        int top = 0;
        do temp[top++] = (x % 10) + '0', x /= 10;
        while (x);
        while (top) *p_out++ = temp[--top];
    }
    void putChar(char c) {
        if (p_out == p_out_end) flush();
        *p_out++ = c;
    }
} io;
...
struct PolyUtils {
    ...
    static constexpr m256i vload(const void* p) {
        return _mm256_load_si256((const m256i*)p);
    }
    static constexpr void vstore(void* p, m256i a) {
        _mm256_store_si256((m256i*)p, a);
    }
    static constexpr m256i vloadu(const void* p) {
        return _mm256_loadu_si256((const m256i*)p);
    }
    static constexpr void vstoreu(void* p, m256i a) {
    ...
    static constexpr Mint I = qpow(G, (P - 1) / 4);
    ...
    size_t g_len = 4, inv_len = 1;

    PolyUtils() { inv[1] = 1; }
    void prepareG(size_t len) {
        for (size_t& i = g_len; i <= len; i <<= 1) {
            size_t s = i / 4;
            Mint j1{1}, step1 = qpow(G, (P - 1) / i);
            Mint j2{1}, step2 = step1 * step1;
            Mint j3{1}, step3 = step2 * step1;
    ...
    static m256i add(m256i x, m256i y) {
        m256i x1 = _mm256_add_epi32(x, y);
        m256i y1 = _mm256_sub_epi32(x1, V_P2);
        return _mm256_min_epu32(x1, y1);
    }
    static m256i sub(m256i x, m256i y) {
        m256i x1 = _mm256_sub_epi32(x, y);
        m256i y1 = _mm256_add_epi32(x1, V_P2);
        return _mm256_min_epu32(x1, y1);
    }
...
#pragma GCC unroll 8
    ...
    static void vbutterfly(m256i& A, m256i& B, m256i& C, m256i& D) {
        m256i t0 = add(A, C), t1 = sub(A, C), t2 = add(B, D),
              t3 = mul(sub(B, D), V_I);
        A = add(t0, t2), B = add(t1, t3), C = sub(t0, t2), D = sub(t1, t3);
    }
    ...
    static m256i vbutterfly4(m256i v) {
        constexpr m256i c = (m256i)__v8su{M.R, I.raw(), P - M.R, (-I).raw(),
                                          M.R, I.raw(), P - M.R, (-I).raw()};
...
#pragma GCC unroll 8
            for (size_t j = 0; j < len; j += 16) {
                m256i v0 = vload(a + j), v1 = vload(a + j + 8);
                v0 = mul(vbutterfly4(v0), rot0);
                v1 = mul(vbutterfly4(v1), rot1);
                vbutterfly16(v0, v1);
                vstore(a + j, v0), vstore(a + j + 8, v1);
            }
            i = 64;
...
#pragma GCC unroll 8
    ...
    // out <- exp(f)
    void polyexp(const Mint* f, size_t len_b, Mint* out) {
        if (!len_b) return;
        ...
        for (size_t k = 1, k2 = 2; k < len; k = k2, k2 <<= 1) {
            ...
            sub(t4 + k, f + k, std::min(len_b, k2) - k, t4 + k);
            ...
            DIT(t1, k2), copy(out + k, k, t1 + k);
            DIT(t2, k2), copy(g + k, k, t2 + k);
        }
    }
};
...
public:
    ...
    FPoly(R&& range) {
        std::vector<Mint> tmp{};
        for (auto&& x: range) tmp.emplace_back(x);
        _len = tmp.size();
...
int main() {
    ...
    int n = io.read();
    Poly f(n, true);
    for (int i = 0; i < n; i++) f[i] = io.read();
    f = f.inv();
    for (auto& i: f) io.write(i()), io.putChar(' ');
    return 0;
}
