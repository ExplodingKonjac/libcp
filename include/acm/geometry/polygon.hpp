...
T area2(const std::vector<Vec2<T>>& p) {
    T s{};
...
std::vector<Vec2<T>> chain(const std::vector<Vec2<T>>& p) {
    std::vector<Vec2<T>> res;
    res.reserve(p.size());
    const auto add = [&](Vec2<T> q) {
        while (
            res.size() >= 2 &&
...
template <GeometryScalar T>
bool outside(Line2<T> l, Vec2<T> p) {
...
class Polygon {
private:
    std::vector<Vec2<T>> p_;

public:
    Polygon() = default;

    explicit Polygon(std::vector<Vec2<T>> p): p_{std::move(p)} {
        if (p_.size() > 1 && p_.front() == p_.back()) p_.pop_back();
        if (detail::area2(p_) < 0) std::reverse(p_.begin(), p_.end());
    }

    const std::vector<Vec2<T>>& vertices() const { return p_; }
    ...
    const Vec2<T>& operator[](usize i) const { return p_[i]; }

    T area() const { return std::abs(detail::area2(p_)) / T{2}; }
    ...
    PointPolygonRelation relation(Vec2<T> q) const {
...
template <GeometryScalar T>
std::vector<Vec2<T>> convex_hull(
    std::vector<Vec2<T>> p, HullMode mode = HullMode::full
) {
...
template <GeometryScalar T>
std::optional<Polygon<T>> half_plane_intersection(std::vector<Line2<T>> ls) {
    ...
    std::vector<Line2<T>> u;
    u.reserve(h.size());
    for (auto x: h) {
        auto l = x.l;
        if (!u.empty() &&
            parallel(u.back(), l) &&
            dot(u.back().direction, l.direction) > T{}) {
    ...
    const auto inter = [&](Line2<T> a, Line2<T> b) -> std::optional<Vec2<T>> {
    ...
    const auto equal = [](Vec2<T> a, Vec2<T> b) {
        if constexpr (std::floating_point<T>) return almost_equal(a, b);
        return a == b;
    };
    std::vector<Vec2<T>> p;
    p.reserve(q.size());
    for (usize i = 0; i != q.size(); i++) {
        if (orientation(
                Vec2<T>{}, q[i].direction, q[(i + 1) % q.size()].direction
            ) <= 0)
            return std::nullopt;
        const auto x = inter(q[i], q[(i + 1) % q.size()]);
        if (!x) return std::nullopt;
        if (p.empty() || !equal(p.back(), *x)) p.push_back(*x);
    }
    if (p.size() > 1 && equal(p.front(), p.back())) p.pop_back();
