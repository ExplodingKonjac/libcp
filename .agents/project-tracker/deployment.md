...
`install.sh` requires an explicit prefix, removes the existing `<PREFIX>/include/cp` directory, recreates it, and copies the repository's `cp/` contents. The project does not produce archives, system packages, containers, or versioned binary releases.
...
|-------|---------|----------|
| Header integration | Compile a representative test with a supported compiler | Successful C++23 compilation |
