...
void require_point_near(
    Vec2<T> actual, Vec2<T> expected, T eps = static_cast<T>(1e-9)
) {
...
void test_circle_primitives() {
    ...
    const Circle2<double> circle{{0, 0}, 5};
    assert(on_circle(circle, Vec2<double>{3, 4}));
    assert(contains(circle, Vec2<double>{0, 0}));
    assert(contains(circle, Vec2<double>{3, 4}));
    assert(!contains(circle, Vec2<double>{6, 0}));

    const Circle2<double> point_circle{{0, 0}, 0};
    assert(!on_circle(point_circle, Vec2<double>{1e-5, 0}));
    assert(!contains(point_circle, Vec2<double>{1e-5, 0}));
}

void test_circle_distances() {
    const Circle2<double> circle{{0, 0}, 5};
    assert(near(distance_to_circle(Vec2<double>{0, 0}, circle), 5.0));
    assert(near(distance_to_circle(Vec2<double>{3, 4}, circle), 0.0));
    assert(near(distance_to_disk(Vec2<double>{0, 0}, circle), 0.0));
    assert(near(distance_to_disk(Vec2<double>{8, 0}, circle), 3.0));
}

void test_line_circle_intersections() {
    const Circle2<double> circle{{0, 0}, 5};

    const auto none = intersection(Line2<double>{{0, 6}, {1, 0}}, circle);
    assert(none.kind == PointIntersectionKind::none);
    assert(none.count() == 0);

    const auto tangent = intersection(Line2<double>{{0, 5}, {1, 0}}, circle);
    assert(tangent.kind == PointIntersectionKind::one);
    assert(tangent.count() == 1);
    require_point_near(tangent.points[0], Vec2<double>{0, 5});

    const auto two = intersection(Line2<double>{{0, 0}, {1, 0}}, circle);
    assert(two.kind == PointIntersectionKind::two);
    assert(two.count() == 2);
    require_point_near(two.points[0], Vec2<double>{-5, 0});
    require_point_near(two.points[1], Vec2<double>{5, 0});
}

void test_circle_intersections() {
    const Circle2<double> first{{0, 0}, 5};

    assert(
        intersection(first, Circle2<double>{{20, 0}, 5}).kind ==
        PointIntersectionKind::none
    );
    assert(
        intersection(first, Circle2<double>{{1, 0}, 1}).kind ==
        PointIntersectionKind::none
    );

    const auto external_tangent = intersection(first, Circle2<double>{{10, 0}, 5});
    assert(external_tangent.kind == PointIntersectionKind::one);
    require_point_near(external_tangent.points[0], Vec2<double>{5, 0});

    const auto internal_tangent = intersection(first, Circle2<double>{{3, 0}, 2});
    assert(internal_tangent.kind == PointIntersectionKind::one);
    require_point_near(internal_tangent.points[0], Vec2<double>{5, 0});

    const auto two = intersection(first, Circle2<double>{{6, 0}, 5});
    assert(two.kind == PointIntersectionKind::two);
    require_point_near(two.points[0], Vec2<double>{3, 4});
    require_point_near(two.points[1], Vec2<double>{3, -4});

    assert(
        intersection(first, Circle2<double>{{0, 0}, 4}).kind ==
        PointIntersectionKind::none
    );
    const auto coincident = intersection(first, Circle2<double>{{0, 0}, 5});
    assert(coincident.kind == PointIntersectionKind::coincident);
    assert(coincident.count() == 0);

    const auto same_point =
        intersection(Circle2<double>{{2, 3}, 0}, Circle2<double>{{2, 3}, 0});
    assert(same_point.kind == PointIntersectionKind::one);
    require_point_near(same_point.points[0], Vec2<double>{2, 3});
...
void test_circle_from_three_points() {
    static_assert(
        std::same_as<
            decltype(circle_from(Vec2<double>{}, Vec2<double>{}, Vec2<double>{})),
            Circle2<double>
        >
    );

    const auto triangle =
        circle_from(Vec2<double>{0, 0}, Vec2<double>{4, 0}, Vec2<double>{0, 3});
    require_point_near(triangle.center, Vec2<double>{2, 1.5});
    assert(near(triangle.radius, 2.5));

    const auto collinear =
        circle_from(Vec2<double>{-3, 2}, Vec2<double>{5, 2}, Vec2<double>{0, 2});
    require_point_near(collinear.center, Vec2<double>{1, 2});
    assert(near(collinear.radius, 4.0));
    ...
    const auto huge = circle_from(
        Vec2<double>{0, 0}, Vec2<double>{1e155, 0}, Vec2<double>{0, 1e155}
    );
...
void test_minimum_enclosing_circle_degenerate_inputs() {
    static_assert(
        std::same_as<
            decltype(minimum_enclosing_circle(std::vector<Vec2<double>>{})),
            std::optional<Circle2<double>>
        >
    );

    assert(!minimum_enclosing_circle(std::vector<Vec2<double>>{}).has_value());

    const auto singleton =
        minimum_enclosing_circle(std::vector<Vec2<double>>{{3, -4}});
    assert(singleton.has_value());
    require_point_near(singleton->center, Vec2<double>{3, -4});
    assert(near(singleton->radius, 0.0));

    const auto duplicate = minimum_enclosing_circle(
        std::vector<Vec2<double>>{{2, 1}, {2, 1}, {2, 1}}
    );
    assert(duplicate.has_value());
    require_point_near(duplicate->center, Vec2<double>{2, 1});
    assert(near(duplicate->radius, 0.0));

    const auto pair =
        minimum_enclosing_circle(std::vector<Vec2<double>>{{-2, 1}, {4, 1}});
    assert(pair.has_value());
    require_point_near(pair->center, Vec2<double>{1, 1});
    assert(near(pair->radius, 3.0));

    const auto collinear = minimum_enclosing_circle(
        std::vector<Vec2<double>>{{-3, 2}, {0, 2}, {5, 2}, {1, 2}}
    );
...
void test_minimum_enclosing_circle_triangles() {
    ...
    assert(contains(Circle2<double>{{0, 0}, 1}, Vec2<double>{1 + 5e-10, 0}));
    assert(!contains(Circle2<double>{{0, 0}, 1}, Vec2<double>{1 + 5e-8, 0}));
}

Circle2<double> reference_pair_circle(Vec2<double> a, Vec2<double> b) {
    const auto center = (a + b) / 2.0;
    return {center, distance(center, a)};
}

std::optional<Circle2<double>> reference_triple_circle(
    Vec2<double> a, Vec2<double> b, Vec2<double> c
) {
...
bool reference_contains_all(
    Circle2<double> circle, const std::vector<Vec2<double>>& points
) {
    return std::ranges::all_of(points, [&](Vec2<double> point) {
        return distance(circle.center, point) <= circle.radius + 1e-8;
    });
}

Circle2<double> brute_force_minimum_circle(
    const std::vector<Vec2<double>>& points
) {
...
void test_minimum_enclosing_circle_against_brute_force() {
    ...
    for (int size = 1; size <= 8; size++) {
        for (int trial = 0; trial != 100; trial++) {
            std::vector<Vec2<double>> points(size);
